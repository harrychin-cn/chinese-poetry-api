...
func (h *WorkImageHandler) Generate(c *gin.Context) {
	...
	if !h.cfg.Enabled {
		failedJob, _ := h.repo.FailImageGenerationJob(job.ID, "image_generation_disabled")
		c.JSON(http.StatusServiceUnavailable, gin.H{
			"error":   "image generation is disabled",
			"code":    "image_generation_disabled",
			"message": "server-side image generation is disabled; dry_run and cached assets remain available without upstream cost",
			"data": gin.H{
				"prompt": formatImagePrompt(*promptRecord),
	...
	assetID := asset.ID
	job, err = h.repo.CompleteImageGenerationJobWithCost(job.ID, &assetID, normalizeImageCostUnits(h.cfg.CostUnits))
	if err != nil {
