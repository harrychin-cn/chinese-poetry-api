...
或输入一句需求</div>
  ...
  <script>
    ...
    async function createKey(silent){var body={name:($('customerName').value||'demo customer'),tier:'trial',daily_limit:1000,notes:'console trial key'};var d=await request('/api/v1/keys',{method:'POST',body:JSON.stringify(body)});var key=d.key||d.api_key||d.token;if(!key)throw new Error('创建 Key 成功但响应里没有 key 字段');$('apiKey').value=key;localStorage.setItem('poetry_api_key',key);if(!silent)toast('API Key 已创建');await refreshStatus();return key}
    ...
    async function refreshStatus(){var k=apiKey();if(!k)return;try{var d=await request('/api/v1/keys/current',{headers:{'X-API-Key':k}});var item=d.key||d.data||d;$('todayUsage').textContent=item.today_usage!=null?item.today_usage:'0';$('dailyLimit').textContent=item.daily_limit!=null?item.daily_limit:'--';$('bindState').textContent=(item.enabled===false?'已停用':'配置可用');var b=await request('/api/v1/billing/status',{headers:{'X-API-Key':k}});$('qanloNotice').textContent='Qanlo 状态：'+((b.qanlo&&b.qanlo.status)||b.status||'可用')+'；configured='+(b.configured!=null?b.configured:'--')}catch(e){toast(e.message)}}
    async function provisionQanlo(){try{var k=await ensureKey();var d=await request('/api/v1/billing/qanlo/provision',{method:'POST',headers:{'X-API-Key':k},body:'{}'});var url=d.url||d.bind_url||d.recharge_url;if(url){window.open(url,'_blank');toast('已打开 Qanlo 绑定页')}else{toast('已生成绑定信息，查看右侧原始返回')}}catch(e){toast(e.message)}}
    async function openRecharge(){try{var k=await ensureKey();var d=await request('/api/v1/billing/qanlo/recharge-session',{method:'POST',headers:{'X-API-Key':k},body:'{}'});var url=d.url||d.recharge_url;if(url){window.open(url,'_blank');toast('已打开 Qanlo 充值页')}else{toast('已生成充值信息，查看右侧原始返回')}}catch(e){toast(e.message)}}
