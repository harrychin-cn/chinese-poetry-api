...
<section class="card"><h2>核心接口</h2><ul><li><code>POST /api/v1/keys</code>：公开入口已禁用创建，返回 403；Key 必须由管理员或 Qanlo 开通链路发放。</li><li><code>GET /api/v1/keys/current</code>：查看当前 Key 和今日用量。</li><li><code>POST /api/v1/billing/qanlo/provision</code>：生成 Qanlo Agent Key 绑定 URL。</li><li><code>POST /api/v1/billing/qanlo/recharge-session</code>：生成 Qanlo 充值 URL。</li><li><code>GET /api/v1/poems/query</code>：诗词复合查询。</li><li><code>GET /api/v1/poems/search/fulltext</code>：全文搜索。</li><li><code>GET /api/v1/knowledge/recall</code>：AI 知识库召回。</li><li><code>POST /api/v1/knowledge/batch</code>：批量知识库召回。</li><li><code>POST /api/v1/images/generate</code>：诗词意境图生成。</li><li><code>POST /api/v1/works</code>：录入原创作品。</li><li><code>GET /api/v1/works</code>：查看自己的作品库。</li><li><code>GET /api/v1/works/:id/plagiarism-report</code>：查看原创查重报告。</li><li><code>POST /api/v1/works/:id/images/generate</code>：按作品生成诗画图片，支持 dry_run 只准备 Prompt。</li><li><code>GET /api/v1/works/:id/media-assets</code>：查看作品图片资产。</li><li><code>GET /api/v1/works/:id/image-jobs</code>：查看作品生图任务。</li><li><code>GET /api/v1/public/works/:code</code>：公开作品展示。</li><li><code>POST /api/v1/feedback</code>：客户反馈。</li></ul></section>
