...
func TestWorkImageGenerateStoresAssetAndJob(t *testing.T) {
	...
	assert.False(t, jobs[0].Cached)
	assert.Equal(t, 2, jobs[0].CostUnits)
}
...
func TestWorkImageGenerateUsesCachedAssetWithoutQuota(t *testing.T) {
	...
	assert.True(t, jobs[0].Cached)
	assert.Equal(t, 0, jobs[0].CostUnits)
	assert.False(t, jobs[1].Cached)
	assert.Equal(t, 2, jobs[1].CostUnits)
}

func TestWorkImageGenerateDisabledRecordsFailedJobWithoutQuota(t *testing.T) {
	router, repo, rawKey, work := setupWorkImageTestRouter(t, config.ImageConfig{
		Enabled:        false,
		APIKey:         "test-image-key",
		BaseURL:        "https://qanlo.test/openai/v1",
	...
	usage, err := repo.GetAPIKeyUsageCount(1, time.Now().UTC().Format("2006-01-02"))
	require.NoError(t, err)
	assert.Equal(t, 0, usage, "disabled image generation must not consume local quota")

	jobs, err := repo.ListWorkImageJobs(1, work.ID, 10)
	require.NoError(t, err)
	require.Len(t, jobs, 1)
	assert.Equal(t, database.ImageJobStatusFailed, jobs[0].Status)
	assert.Equal(t, 0, jobs[0].CostUnits)
	assert.False(t, jobs[0].Cached)
}
