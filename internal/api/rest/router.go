...
func SetupRouter(cfg *config.Config, db *database.DB, repo *database.Repository) *gin.Engine {
	...
	router.GET("/service-worker.js", handler.ServiceWorkerJS)
	router.GET("/pwa-icon.svg", handler.PWAIconSVG)
	router.GET("/favicon.ico", handler.PWAIconSVG)
	...
	router.GET("/library", handler.LibraryPage)
	router.GET("/certificates/:code", handler.CertificatePage)
	...
	v1 := router.Group("/api/v1")
	{
		...
		v1.GET("/wallet", withAPIKey(middleware.APIKeyAuthNoUsage(repo), walletHandler.Get)...)
		v1.GET("/wallet/transactions", withAPIKey(middleware.APIKeyAuthNoUsage(repo), walletHandler.Transactions)...)
		v1.POST("/wallet/top-up", withAPIKey(middleware.APIKeyAuthNoUsage(repo), walletHandler.TopUp)...)
		...
		certificateHandler := handler.NewCertificateHandler(repo)
		libraryHandler := handler.NewLibraryHandler(repo)
		v1.GET("/public/works", libraryHandler.ListPublicWorks)
		v1.GET("/public/works/:code", workHandler.PublicGet)
		v1.GET("/public/works/:code/certificate", certificateHandler.PublicGet)
		v1.GET("/public/users/:handle", accountHandler.PublicProfile)
		v1.GET("/public/users/:handle/works", accountHandler.PublicWorks)
		v1.GET("/public/rankings/works", libraryHandler.WorkRankings)
		v1.GET("/public/rankings/authors", libraryHandler.AuthorRankings)
		v1.GET("/partners/works/export", withAPIKey(middleware.APIKeyAuthNoUsage(repo), libraryHandler.PartnerExport)...)
		...
		v1.POST("/works/:id/tip", withAPIKey(middleware.APIKeyAuthNoUsage(repo), walletHandler.TipWork)...)
		v1.GET("/works/:id/tips", withAPIKey(middleware.APIKeyAuthNoUsage(repo), walletHandler.ListWorkTips)...)
		v1.GET("/works/:id/certificate", withAPIKey(middleware.APIKeyAuthNoUsage(repo), certificateHandler.Get)...)
		v1.POST("/works/:id/certificate", withAPIKey(middleware.APIKeyAuthNoUsage(repo), certificateHandler.Issue)...)
		v1.POST("/works/:id/certificate/anchor", withAPIKey(middleware.APIKeyAuthNoUsage(repo), certificateHandler.Anchor)...)
