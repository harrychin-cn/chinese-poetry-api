...
1. **成交路径更短**：客户创建 API Key 后，直接跳 Qanlo 精简充值页，充值回来就能用。
...
4. **充值链路**：客户端/控制台创建或绑定 API Key -> 跳转 Qanlo 精简充值页 -> 充值完成回到客户端/控制台 -> 立即可调用。
...
| 接口 | 用途 | 阶段 |
| --- | --- | --- |
| `POST /api/v1/keys` | 客户端自助创建本项目 API Key，返回明文一次，数据库只存 hash | 已实现 |
...
| API Key 鉴权 | 已实现 | 支持客户端创建、管理员创建/列表/吊销、启停、备注、每日限额调整、请求中间件、今日用量统计 |
...
| 最小控制台 | 已实现 MVP | `/console` 支持创建 Key、跳转绑定/充值、刷新状态、试调用、客户 Key 管理、AI 抽检、用量趋势、接口错误率和热门查询 |
...
- [x] 明确增强版暂不开源。
- [x] 明确只保留 QanloAPI 入口。
- [x] 明确充值链路：创建/绑定 API Key -> Qanlo 精简充值 -> 回跳 -> 可用。
- [x] 修正开发规划文档。
...
- [x] 明确免费试用默认限额：`API_DEFAULT_DAILY_LIMIT`。
...
目标：完成商业闭环最关键的一步：客户能创建 Key、充值、返回后直接调用。
...
- [x] 增加客户端可直接创建 Key 的入口：`POST /api/v1/keys`。
- [x] 控制台/客户端前端按钮：创建 Key、充值、刷新状态。
...
5. 后台和文档站必须服务转化：让客户能创建 Key、充值、调用、查看用量。
